          ...
          <div className="bg-white border border-emerald-200 p-8 text-center shadow-sm rounded-xl">
            ...
            {pixStatus === 'paid' ? (
                ...
                <div className="animate-in zoom-in duration-500">
                    ...
                    <h1 className="text-3xl font-serif font-medium text-emerald-900 mb-2">Pagamento Recebido!</h1>
                    <p className="text-gray-600 mb-6">Seu pedido <strong>{orderData.orderId}</strong> foi confirmado.</p>
                ...
                <>
                    ...
                    <h1 className="text-3xl font-serif font-medium text-emerald-900 mb-2">Pedido Gerado!</h1>
                    <p className="text-gray-600 mb-6">Número do pedido: <strong>{orderData.orderId}</strong></p>
              ...
              {step === "shipping" && (
                <form onSubmit={handleShippingSubmit} className="space-y-6">
                  ...
                  {showShippingOptions && (
                    <div className="space-y-3 mt-6 bg-stone-50 p-4 rounded-lg border border-stone-100">
                        ...
                        {shippingOptions.map((option) => (
                            <label key={option.id} className={`flex items-center p-4 border rounded-lg cursor-pointer bg-white ${selectedShipping?.id === option.id ? 'border-emerald-500 ring-1 ring-emerald-500' : 'hover:border-emerald-300'}`}>
                                <input type="radio" name="shipping" checked={selectedShipping?.id === option.id} onChange={() => selectShipping(option)} className="mr-4" />
                                <div className="flex-1 flex justify-between">
                                    <span className="font-bold">{option.name}</span>
                                    <span className="font-bold text-emerald-700">R$ {option.price.toFixed(2)}</span>
                                </div>
                            </label>
                        ))}
                    </div>
                  ...
                  <div className="flex justify-end pt-4">
                      <Button type="submit" size="lg" disabled={!selectedShipping} className="bg-emerald-700 hover:bg-emerald-800 text-white">Ir para Pagamento</Button>
                  </div>
                </form>
              )}
                ...
                <form onSubmit={handlePaymentSubmit} className="space-y-6">
                  ...
                  <div className="flex justify-between pt-6 border-t mt-6">
                      ...
                      <Button type="submit" size="lg" disabled={isProcessing} className="bg-emerald-700 hover:bg-emerald-800 text-white min-w-[200px]">
                        {isProcessing ? <><Loader2 className="mr-2 h-4 w-4 animate-spin"/> Processando...</> : 'Finalizar Pedido'}
                      </Button>
                  </div>
                </form>
              )}
