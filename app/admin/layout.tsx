...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!isLoading && (!user || (currentUser?.role !== 'admin' && currentUser?.user_metadata?.role !== 'admin'))) {
         // Lógica de proteção
    }
        ...
        <div className="flex flex-col gap-10">
          ...
          <div className="flex items-center gap-3 px-2">
            ...
            <div className="flex flex-col">
              <h1 className="text-[#1A1A1A] text-lg font-bold font-serif leading-none">Caiçara Mix</h1>
              <p className="text-[#4A6B53] text-xs font-medium leading-normal mt-0.5">Painel Admin</p>
            </div>
          </div>
          ...
          <nav className="flex flex-col gap-1">
            {menuItems.map((item) => (
              ...
              >
                <item.icon className={`h-5 w-5 ${isActive(item.href) ? "text-white" : "text-current"}`} strokeWidth={2} />
